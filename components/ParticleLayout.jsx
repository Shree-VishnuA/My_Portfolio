...
export default function ParticleField() {
  ...
  // Thresholds are based on UNSCALED baseSize (5–35 range) so the
  // small/medium/large classification stays consistent across breakpoints.
  const MEDIUM_SIZE_THRESHOLD = 12;
  const LARGE_SIZE_THRESHOLD = 22;
...
const darkColors = ["#020203", "#050506", "#08080A", "#0B0B0D", "#0E0E10", "#111113", "#141416", "#171719"];
  ...
  : "rgba(0, 0, 0, 0.08)";
    ...
    const animate = () => {
      ...
      particles.current.forEach((p, i) => {
        ...
        // Medium/large particles get a medium-thickness border in both themes.
        if (p.baseSize >= MEDIUM_SIZE_THRESHOLD) {
          ctx.lineWidth = Math.max(1, 1.5 * particleSize);
          ctx.strokeStyle = borderColor;
          ctx.stroke();
        }

        void LARGE_SIZE_THRESHOLD; // reserved for future size-tiered styling
